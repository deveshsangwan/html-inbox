...
```sh
html-inbox remote revoke --yes
```

In an interactive terminal, omit `--yes` to confirm at the prompt. If the command preserves intent or loses its response, run `html-inbox remote reconcile` instead of issuing another revoke.
