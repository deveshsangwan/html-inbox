...
export class RemoteWorkflow {
  ...
  private async revokeUnlocked(): Promise<RemoteRevokeResult> {
    await this.assertNoOperation();
    const state = await this.requireState();
    const revokedUrl = state.lastDeployment?.receipt.productionInboxUrl ?? "";
  ...
  async reconcile(): Promise<RemoteState> {
    return this.withMutationLock(() => this.reconcileUnlocked());
  }

  private async reconcileUnlocked(): Promise<RemoteState> {
    ...
    const existing = deployments
      .filter(
        (deployment) =>
          ...
          (!deployment.branch || deployment.branch === operation.branch),
      )
  ...
  private async finalizeOperation(operation: RemoteOperation): Promise<RemoteState> {
    ...
    if (operation.kind === "init") {
      ...
      if (
        current.capability === operation.capability &&
        current.lastDeployment?.snapshotHash === operation.snapshotHash
      ) {
...
async function acquireRemoteLock(lockPath: string): Promise<() => Promise<void>> {
  ...
  throw new Error("Could not acquire the HTML Inbox remote mutation lock");
}
