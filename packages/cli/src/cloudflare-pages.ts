  ...
  async run(invocation: CommandInvocation): Promise<CommandResult> {
    return new Promise((resolve, reject) => {
      ...
      const child = spawn(invocation.command, invocation.args, {
        ...
        shell: process.platform === "win32" && invocation.command === "npx",
  ...
  async deploySnapshot(
    ...
    try {
      ...
      return {
        ...
        productionInboxUrl: joinInboxUrl(urls.productionUrl, snapshot.inboxPath),
      };
    } finally {
...
export function createWranglerInvocation(
  ...
  return {
    ...
    cwd,
    env: { CLOUDFLARE_ACCOUNT_ID: normalizeCloudflareAccountId(accountId) },
    timeoutMs,
  };
}
...
export function parseWranglerDeployUrls(output: string): {
  ...
  return {
    deploymentUrl,
    productionUrl: `https://${labels.slice(1).join(".")}`,
  };
}
...
export function receiptFromDeployment(
  ...
  return {
    ...
    productionInboxUrl: joinInboxUrl(urls.productionUrl, inboxPath),
  };
}
...
async function copyStaticTree(
  ...
  for (const entry of entries) {
    ...
    await writePrivateFile(destinationPath, await readFile(sourcePath));
  }
}
