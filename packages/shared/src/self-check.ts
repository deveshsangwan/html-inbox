...
// Tier 1: the security boundary. These reach the user through gaps the
...
for (const html of [
  ...
  // CSP has no navigation directive and the sandbox permits self-navigation.
...
assert.equal(validateHtml('<html><img src="data:image/png;base64,iVBORw0KGgo="></html>').ok, true);
assert.equal(
  validateHtml(
